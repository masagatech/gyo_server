...
psngr.getPassengerReports = function getPassengerReports(req, res, done) {
    db.callProcedure("select " + globals.schema("funget_rpt_passengerdetails") + "($1,$2::json);", ['rptpsngr', req.body], function(data) {
        rs.resp(res, 200, data.rows);
    }, function(err) {
        rs.resp(res, 401, "error : " + err);
    }, 1)
